...
const Projects = () => {
  ...
  const [repos, setRepos] = useState([])
    ...
    useEffect(async () => {
        const res = await fetch('https://api.github.com/users/robonexx/starred?page=1&per_page=5')
        ...
        {/*  <InfoContainer  /> */}
         {
                                    repos.map(({name, html_url, description, id}) => {
                                        return (
                                          <InfoContainer name={name} html_url={html_url} description={description} key={id} count={count} setCount={setCount}/>
